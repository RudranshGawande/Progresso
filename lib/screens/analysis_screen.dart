...
class _AnalysisScreenState extends State<AnalysisScreen> {
  ...
  _PeriodStats _gatherPeriodStats(List<Goal> goals, DateTimeRange range, {String? period}) {
    ...
    for (var s in periodSessions) {
      // Always aggregate by session/goal level regardless of period
      final key = s.displayName;
      allocation[key] = (allocation[key] ?? 0) + (s.session.duration.inMinutes / 60.0);
    }
  ...
  Widget _buildProductivityTrendsCard(List<double> chartData, List<String> chartLabels) {
    ...
    if (_selectedPeriod == 'Today') {
      ...
      trendValue = chartData.isEmpty ? '0%' : 'High +${(chartData.reduce(max) * 100).toInt()}%';
    } else if (_selectedPeriod == 'Month') {
      title = 'Weekly Focus Comparison';
      description = 'Focus intensity trends across the month';
      trendLabel = 'MONTH TREND';
      trendValue = 'Growth ${chartData.isEmpty ? "0%" : _calculateTrend(chartData.isEmpty ? 0 : chartData.last, chartData.isEmpty ? 0 : chartData.first)}';
    } else if (_selectedPeriod == 'Year') {
      title = 'Yearly Productivity Trends';
      description = 'Monthly focus distribution across the year';
      trendLabel = 'ANNUAL TREND';
      trendValue = 'Change ${chartData.isEmpty ? "0%" : _calculateTrend(chartData.isEmpty ? 0 : chartData.last, chartData.isEmpty ? 0 : chartData.first)}';
    } else {
      title = 'Daily Productivity Trends';
      description = 'Daily focus time over the last week';
      trendLabel = 'WEEK TREND';
      trendValue = 'Change ${chartData.isEmpty ? "0%" : _calculateTrend(chartData.isEmpty ? 0 : chartData.last, chartData.isEmpty ? 0 : chartData.first)}';
    }
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Icon(Icons.trending_up, color: AppColors.emerald500, size: 14),
                      const SizedBox(width: 4),
                      Text(
                        trendValue,
                        style: GoogleFonts.inter(
      ...
      child: Column(
        ...
        children: [
          ...
          if (allocation.isEmpty)
             Center(child: Text('No data for this period', style: TextStyle(color: AppColors.slate400, fontSize: 13))),
          ...List.generate(allocation.length, (index) {
            final entry = allocation[index];
            final name = entry.key;
            final hours = entry.value;
            final percent = totalHours > 0 ? ((hours / totalHours) * 100).toInt() : 0;
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text('Summary of focus blocks across the week', style: TextStyle(fontSize: 14, color: AppColors.slate500)),
                ],
              ),
              Row(
                children: [
          ...
          SizedBox(
            ...
            child: Row(
               ...
               children: List.generate(7, (i) {
                 final dayHours = dailyHours[i];
                 final dayCount = dailyCount[i];
                 // Normalize height based on max hours in the week (at least 4 hours for full height)
                 final maxH = dailyHours.reduce(max);
                 final hFactor = maxH > 0 ? (dayHours / max(maxH, 4.0)) : 0.0;
          ...
          Row(
            ...
            children: [
              _buildMetricDetail('TOTAL SESSIONS', '$totalSessions'),
              const SizedBox(width: 80),
              _buildMetricDetail('MOST ACTIVE DAY', totalSessions > 0 ? mostActiveDay : '--'),
              const SizedBox(width: 80),
              _buildMetricDetail('AVG FOCUS/SESSION', totalSessions > 0 ? '${(dailyHours.fold(0.0, (a, b) => a + b) / totalSessions * 60).toInt()}m' : '--', valueColor: AppColors.emerald500),
            ],
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.start,
            children: [
              _buildMetricDetail('TODAY FOCUS', '${sessions.fold(0.0, (a, b) => a + (b.session.duration.inMinutes / 60.0)).toStringAsFixed(1)}h'),
              const SizedBox(width: 80),
              _buildMetricDetail('CORE SESSIONS', '${sessions.length}'),
              const SizedBox(width: 80),
              _buildMetricDetail('AVG INTENSITY', sessions.isEmpty ? '0%' : '${(sessions.fold(0.0, (a, b) => a + b.session.intensity) / sessions.length * 100).toInt()}%', valueColor: AppColors.emerald500),
            ],
          ),
        ],
      ),
          ...
          Positioned.fill(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  if (sessions.isEmpty) {
                    return Center(child: Text('No activity data for today', style: TextStyle(color: AppColors.slate400, fontSize: 12)));
                  }
                  
                  // Sort sessions by time
                  final sorted = List<SessionWithMeta>.from(sessions)
                    ..sort((a, b) => a.session.timestamp.compareTo(b.session.timestamp));
                  
                  // For "Today", we map 00:00 - 24:00 to 0.0 - 1.0
                  // But let's focus on the 9AM-9PM typical window for better visualization if desired
                  // or just the whole day.
                  
                  return Stack(
                    children: sorted.map((s) {
                      final startHours = s.session.timestamp.hour + (s.session.timestamp.minute / 60.0);
                      final durationHours = s.session.duration.inMinutes / 60.0;
                      
                      final left = (startHours / 24.0) * width;
                      final blockWidth = (durationHours / 24.0) * width;
                      
                      return _buildTimelineBlockWithLabel(
                        left, 
                        max(blockWidth, 4.0), 
                        AppColors.emerald500, 
                        '${DateFormat('HH:mm').format(s.session.timestamp)}: ${s.taskName}'
                      );
                    }).toList(),
                  );
                },
    ...
    return Positioned(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Lunch',
            style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.slate400),
          ),
        ],
      ),
    );
  ...
  Widget _buildTimelineBlock(double left, double width, Color color, String timeLabel) {
    return Positioned(
      left: left,
      top: 34,
      width: width,
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Tooltip(
                message: focusHours > 0 
                    ? 'Day $dayNum: ${focusHours.toStringAsFixed(1)}h focus duration' 
                    : 'Day $dayNum: No focus records',
                decoration: BoxDecoration(
                  color: AppColors.slate800,
                  borderRadius: BorderRadius.circular(8),
                ),
          ...
          Row(
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('LONGEST STREAK', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.slate400, letterSpacing: 1.0)),
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('AVG DAILY FOCUS', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.slate400, letterSpacing: 1.0)),
                  const SizedBox(height: 8),
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final displayData = data.isEmpty ? [0.0] : data;
